...
const SearchModal = ({ open, setOpen }: { open: boolean; setOpen: any }) => {
          ...
          <div className="">
            ...
            <div className="flex space-x-2 overflow-x-auto pb-2 sm:grid sm:grid-cols-4 sm:space-x-0 sm:gap-2">
              {RecommendedProducts.map((product, index: number) => (
                <div
                  key={index}
                  className="space-y-2 min-w-27.5 shrink-0 sm:min-w-0"
                >
                  <div className="aspect-square relative">
                    <img
                      src={product.image}
                      alt={product.name}
                      className="absolute inset-0 w-50 h-full rounded-none"
                    />
