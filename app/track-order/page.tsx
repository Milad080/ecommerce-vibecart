...
const TrackOrderPage = () => {
  ...
  const handleSubmit = async (e: React.SubmitEvent) => {
    ...
    if (!orderId.trim()) {
      setError("please insert an order number");
      setLoading(false);
      return;
    }
    try {
      ...
      setError(
        "An error occurred while looking up your order. Please try again",
      );
    } finally {
      setLoading(false);
    }
    ...
    <div className="min-h-screen bg-gray-100 p-4 flex items-center justify-center">
      <div className="bg-white p-4 sm:p-6 lg:p-8 rounded-lg shadow-lg w-full max-w-md">
        ...
        {error && (
          <div
            role="alert"
            className="mt-4 p-3 bg-red-100 border-red-400 text-red-700 rounded text-sm"
          >
            {error}
          </div>
        )}
